...
**You can also bring your own data.** Upload a spreadsheet and ask questions about it in the same conversation — Skylark reads both connected business systems and files you provide, and always tells you which one an answer came from.
...
Alongside the monday.com boards, Skylark can analyse spreadsheets you upload.

**Supported formats:** CSV, TSV, XLSX, XLS and ODS. Not "any file" — PDFs,
images, JSON and Parquet are different kinds of data source and are not read.

**How it works — the same trust model as the boards:**

```
your file → parsed and type-inferred server-side
          → normalised, with a data-quality report
          → compact snapshot held by your browser
question  → deterministic query runs in code
          → compact result + coverage → the model → your answer
```

The model never receives your rows. It picks a tool, reads the computed result,
and writes the explanation — exactly as it does for monday.com. It cannot total
a column itself because it never sees the column.

**What Skylark tells you about your data.** Every column reports how many rows
carry a usable value, and separates *blank* from *unreadable*. A total always
arrives with its coverage:

> Revenue totals ₹5.8 L across 5 of 7 rows; 2 have no value and are excluded
> rather than counted as zero.

Blank is never zero — the same rule the boards get.

**Duplicate rows are kept, not removed.** On the monday.com boards a repeated
row is an export artefact, so it is dropped. In your spreadsheet two identical
rows are usually two real events — two invoices for the same amount on the same
day — so they are counted and flagged rather than silently removed. You decide
what they mean, not Skylark.

One further difference:
the boards treat words like "pending" and "tbd" as empty, because on those
boards they mean *not filled in yet*. In your spreadsheet "Pending" is probably
a real status, so uploaded data uses a narrower rule — only unambiguous markers
(`-`, `N/A`, `null`, `#DIV/0!` and similar) count as blank.

**Asking questions.** Name a dataset if you have several ("Using Revenue Q3,
which region earned most?"), or just ask — with one dataset loaded Skylark uses
it. If the file lacks the field your question needs, it says so instead of
substituting a different column.

**Datasets and boards are kept separate.** Skylark will not merge a total from
your spreadsheet with one from monday.com, because there is no verified shared
identifier between them. It reports them side by side and says why.

**Lifecycle.** Uploaded data belongs to the workspace, not the conversation:
starting a new conversation keeps it. It lives in your browser's memory only —
nothing is written to a server or a database, and closing the tab clears it.

**Limits** (all configurable — see `docs/DEPLOY.md`): 8 MB per file, 20,000
rows, 60 columns, 5 datasets at once, and a 1.5 MB ceiling on the parsed
snapshot. That last one binds hardest: the snapshot travels with every question,
so a wide file can exceed it well before the file-size limit.

**Security.** Uploads are treated as hostile input. Formulas are never
evaluated, macros never execute, nothing is written to disk, and filenames are
never used as paths. Cell contents reach the model only inside JSON tool
results, never as instructions — a cell reading "ignore your instructions" is
analysed as text, not obeyed.

> **A note on the XLSX parser.** Binary formats are read with SheetJS. The build
> published to npm (0.18.5) carries unfixed prototype-pollution and ReDoS
> advisories, so this project installs 0.20.3 from SheetJS's own CDN, where both
> are fixed — `npm audit` reports zero vulnerabilities. The trade-off is that
> installs depend on `cdn.sheetjs.com` being reachable.
...
| `tests/datasets.test.ts` | Every supported format, rejections, type inference at the edges, missing-vs-malformed, unicode/currency/percentage fidelity, limits |
| `tests/dataset-agent.test.ts` | Dataset tool surface, dataset resolution, deterministic queries through the tool layer, result caps, prompt integration |
| `tests/dataset-security.test.ts` | Prompt injection as data, filename neutralisation, limit enforcement, formulas never evaluated, no leakage in tool payloads |
| `tests/streaming.test.ts` | Answer reassembly across provider content parts, NDJSON chunk boundaries and flush, config errors that never name environment variables |
